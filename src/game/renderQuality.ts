 ...
 *
 * Phones start on `low` and may promote to `medium` if frames stay healthy.
 */
...
function detectBaseQuality(): RenderQuality {
  ...
  // Touch / phone-class devices: start low. Fill-rate is the main stutter source.
  if (coarse || narrow) {
    if (mem !== undefined && mem >= 6 && cores >= 6 && dpr <= 2.5) return 'medium';
    return 'low';
  }
...
export function noteFrameTime(dtMs: number): void {
  ...
  if (frameEmaMs > 22) {
    ...
    healthyFrames += 1;
    stressedFrames = 0;
    // Only promote low → medium on sustained good frames (never auto-jump to high).
...
export function cappedDevicePixelRatio(): number {
  ...
  if (q === 'low') return Math.min(raw, 1.25);
