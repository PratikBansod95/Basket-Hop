...
export function fetchLeaderboard(playerId: string, limit = 25): Promise<LeaderboardResponse> {
  const params = new URLSearchParams({
    limit: String(limit),
  });
  if (playerId) params.set('playerId', playerId);
  return requestJson<LeaderboardResponse>(`/api/leaderboard?${params.toString()}`);
}
